...
describe('DoculaConsole', () => {
	...
	it('should be able to parse process argv', () => {
		const c = new DoculaConsole();
		const result = c.parseProcessArgv(['node', 'docula', 'build', '-w', '-s', './site', '-o', './site/dist', '-p', '8080', '-t', './site/template']);
	...
	it('should be able to parse process templatePath', () => {
		const c = new DoculaConsole();
		const result = c.parseProcessArgv(['node', 'docula', 'build', '--templatePath', './site/dist', '-p', '8080']);
		expect(result.command).toEqual('build');
