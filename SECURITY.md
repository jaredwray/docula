...
- Every action is pinned to a full commit SHA. CI workflows default to read-only `contents` permissions, and checkouts that do not push set `persist-credentials: false`. Socket Firewall wraps package installs; workflows are security-linted with zizmor on every PR. Artifact-publishing jobs set `package-manager-cache: false` on `actions/setup-node` so default caches cannot poison release artifacts.
