...
| `cookieAuth` | `{ loginUrl: string; logoutUrl?: string; authCheckUrl?: string; authCheckMethod?: string; authCheckUserPath?: string }` | - | Cookie-based auth. Shows a Login/Logout button in the header based on cookie authentication. See [Cookie Auth](/docs/cookie-auth). |
